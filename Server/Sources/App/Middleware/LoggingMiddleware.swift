...
struct LoggingMiddleware: AsyncMiddleware {
    ...
    func respond(to request: Request, chainingTo next: AsyncResponder) async throws -> Response {
        ...
        do {
            ...
            let _ : Firestore.Document<FirestorePackageHistoryItem> = try await client.createDocument(path: "requests", name: UUID().uuidString, fields: packageHistoryItem.asFirestoreHistoryItem()).get()
        } catch {
            print(error)
